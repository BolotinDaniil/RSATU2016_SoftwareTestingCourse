...
\begin{center}
    ...
    \end{figure}
\end{center}
После запуска программы пользователю требуется пройти авторицазию или регистрацию, после чего ему доступны 2 метода шифрования. Во время работы можно сохранить/загрузить текстовый файл.
...
В окне должна появиться надпись "Неверный пароль!" красного цвета. Этот и последующие тесты класса Login описаны в классе LoginUiTest (Листинг \ref{listing_login:test_wrong_password}).
...
Должно появиться сообщение "Такого пользователя не существует" (Листинг \ref{listing_login:test_wrong_user}).
...
% чтобы данный зоголовок не был пронумерован
